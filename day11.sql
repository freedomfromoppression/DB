...
--| : 또는, () : 패턴그룹
-- J로 시작하며, 세번쨰 문자가 m or n 직원의 이름조회
SELECT emp_name
FROM employees
...
--공백을 기준으로 첫번째 단어 출력
SELECT mem_add1
      ,REGEXP_SUBSTR(mem_add1, '[^ ]+',1,2) as sido --매개변수 3,4 디폴트1
FROM member;
-- 중간 단어 출력(패턴이 없으면 null)
SELECT mem_add1
      ,REGEXP_SUBSTR(mem_add1, '[^ ]+',1,2) as gungu --매개변수 3,4 디폴트1
FROM member;
...
FROM employees
WHERE REGEXP_LIKE(phone_number, '(\d\d)\1$'); --\1은 첫번쨰 캡처 그룹을 다시참조
                                              --즉 이전에 매칭된 두자리 숫자와 정확히
                                              --일치한 두자리 숫자를 의미함.
--DBA 권한이있는 system 계정에서 'study' 계정 생성, 권한설정 후 
--study계정으로 접속하여 table create 문과 insert 문을 순서대로 실행하시오
